...
class ContextAwareWavKANBlock(nn.Module):
    ...
    def __init__(self, d_model, seq_len, dropout=0.1, num_wavelets=8,
        ...
        # self.adaptive_kan = StandardMLPLayer(in_features=d_model, out_features=d_model, seq_len=seq_len, num_wavelets=num_wavelets) # Sử dụng MLP tiêu chuẩn để mô phỏng Wav-KAN, đảm bảo so sánh công bằng về số lượng tham số và sức mạnh biểu diễn.
    ...
    def forward(self, x, debug_store=None, block_idx=None):
        ...
        next_x = self.norm2(x_context) # Add & Norm
